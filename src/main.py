...
url = "https://www.brightnetwork.co.uk/search/?content_types=jobs&career_path_sectors=Technology+%26+IT+Infrastructure&job_types=Internship&sort_by=recent"
...
def get_next_page(soup):
    ...
    if pagination_container:
        ...
        if "Next" in next_page_tag.get_text(strip=True) and next_page_tag.has_attr('href'):
            ...
            return f"{base_url}{next_page_url}"
...
while current_url:
    ...
    for job_link in job_links:
      #storing job descriptions
        page_parts = parse_page(job_link)
...
df['Role'] = df['Role'].apply(lambda x: make_hyperlink(x[0], x[1]))
